...
/**
 * Incremental encrypt session: plaintext in through {@link #write},
 * wire out through {@link #read} / {@link #drainTo}. A session is a
 * dumb byte pump — all chunking, MAC, envelope, and wire-format
 ...
 * {@link Pipeline#encryptStream} or scoped via
 * {@link Pipeline#encrypting}.
 */
@CompileStatic
final class EncryptStream implements AutoCloseable {

    /** The parent Pipeline this session operates on; the field pins
     * it against GC while the session is live. */
    final Pipeline parent

    private final JEncryptStream underlying

    protected EncryptStream(Pipeline parent, JEncryptStream underlying) {
        this.parent = parent
